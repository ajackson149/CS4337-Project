...
def importCsv(conn, tableName, csvPath, columns):
    """Generic CSV import used for tables that don't need special handling."""
    ...
    with csvPath.open("r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if len(ssn) >= 4:
                ...
                password = "password"  # fallback if SSN is weird
...
def importData(conn):
    ...
    if isTableEmpty(conn, "BOOK_AUTHORS"):
        importCsv(conn, "BOOK_AUTHORS", DATA_DIR/"book_authors.csv",
                  ["Isbn", "Author_id"])
